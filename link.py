...
downlinkFrequency = 1
turnAroundRatio = 1
lossfactorTransmitter = 1
groundPower = 1
orbitAltitude = 1
atmosphericAttenuation = 1
diameterSC = 1
uplinkDatarate = 1
case = 1  # check adsee cause idk what this was 
elongationAngle = 1
...
link_labels = ["snr", "Eirp", "POINTINGLOSSGROUND", "spaceLoss", "atmosphereLoss", "gt", "lossDatarate", "boltzmannGain"]

print("==========================UPLINK==========================")
for i, up in enumerate(uplink(diameterGround, downlinkFrequency, turnAroundRatio, lossfactorTransmitter, groundPower, orbitAltitude, 
                              atmosphericAttenuation, diameterSC, uplinkDatarate, case, elongationAngle)):
    print(f"{link_labels[i]}: {up}")

print("==========================DOWNLINK==========================")
for j, down in enumerate(downlink(diameterSC, downlinkFrequency, diameterGround, lossfactorTransmitter, scPower, orbitAltitude, atmosphericAttenuation, 
                                  swathWidthAngle, bitsPixel, pixelsizeangle, pointingInacuracy, case, dutyCycle, downlinkTime, elongationAngle)):
    print(f"{link_labels[j]}: {down}")

print("==========================DATA-RATE==========================")
print(f"rbr:{dataRateSC(swatWidthAngle, altitude, bitsPerPixel, pixelSizeArcmin, dutyCycle, downlinkTime, case)}")
