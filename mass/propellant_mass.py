...
def prop_mass_multiTarget(m_dry, n_targets, dv_list):
...
m_prop = 0  # initialising our propellant mass estimates
m_prop_prev = 1 
while abs(m_prop - m_prop_prev) > 1e-6:  # convergence condition
    m_prop_prev = m_prop  # setting the previous estimate so we can compare
    m_prop, m_prop_list = prop_mass_multiTarget(m_dry + rcs_m(m_prop), n_targets, dv_list)
