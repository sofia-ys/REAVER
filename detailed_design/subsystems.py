...
class Subsystem():
    ...
    def __init__(self, contingency=None):
        self.DEFAULT_CONTINGENCY = 0.2  # default contingency margin for a subsystem 
        self.contingency = contingency if contingency is not None else self.DEFAULT_CONTINGENCY

    def mass_CBE(self):
        '''Total subsystem mass [kg] -- current best estimate'''
        mass = sum(item.mass_kg for item in self._base_mass_items())
        return mass 
    
    def power_CBE(self):
        '''Total subsystem power [W] -- current best estimate'''
        power = sum(item.power_w for item in self._base_power_items())
        return power 
    
    def _apply_contingency(self, base):
        '''Return value including a subsystem contingency margin'''
        return base * (1.0 + self.contingency)
...
class Propulsion(Subsystem):
    ...
    def _base_power_items(self):
        if self.sc_type == "tug":
            return [Component("tug_propulsion_power", self.power)]
