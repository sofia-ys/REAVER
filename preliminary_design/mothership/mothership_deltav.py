...
"""""
#orbits = [rh_orbit, d1_orbit, d2_orbit, d3_orbit, d4_orbit, d5_orbit]
orbits = [rh_orbit, d1_orbit]  # list of tuples we can iterate over

n_targets = 1  # CHANGE THIS IF ADDING MORE DEBRIS
...
# RUN HERE
optimal_path = tot_dv.index(min(tot_dv))
opt_sequence = debris_list[optimal_path]
opt_dvs = deltav_list[optimal_path]

# build readable route string: RH -> D1 -> D3 -> ... -> RH
visit_labels = ["RH"] + [f"D{d}" for d in opt_sequence] + ["RH"]
visit_str = "  ->  ".join(visit_labels)

print("=" * 55)
print("             OPTIMAL DEBRIS COLLECTION ROUTE")
print("=" * 55)
print(f"  Route:     {visit_str}")
print(f"  Total dv:  {tot_dv[optimal_path]:.4f} km/s")
print()
print("  dv per manoeuvre leg:")
for k, (frm, to, dv) in enumerate(zip(visit_labels[:-1], visit_labels[1:], opt_dvs), start=1):
    print(f"    Leg {k}  ({frm} -> {to}):  {dv:.4f} km/s")
print("=" * 55)

# tug return delta V: dv needed for each of the first 4 debris stops to return directly to RH
print()
print("=" * 55)
print("              TUG RETURN DELTA V TO RH")
print("=" * 55)
n_tugs = 1
for k in range(n_tugs):
    debris_label = visit_labels[k + 1]          # D5, D3, D1, D4
    debris_index = opt_sequence[k]              # index into orbits[]
    tug_dv = float(deltaV(orbits[debris_index], rh_orbit))
    print(f"  Tug {k+1}  ({debris_label} -> RH):  {tug_dv:.4f} km/s")
print("=" * 55)